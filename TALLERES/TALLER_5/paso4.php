...
function buscarLibros($biblioteca, $termino)
{
    ...
    foreach ($biblioteca as $libro) {
        if (
            str_contains(strtolower($libro['titulo']), strtolower($termino))
            || str_contains(strtolower($libro['autor']), strtolower($termino))
        )
            $resultado[] = $libro;
    }
    return $resultado;
...
{
    $reporte = [];
    $reporte['Total'] = count($biblioteca);
    $reporte['Prestados'] = array_filter($biblioteca, function ($libro) {
        return $libro['prestado'];
    });
    $reporte['PorGenero'] = PorGenero($biblioteca);
    $reporte['MayorAutor'] = MayorAutor($biblioteca);
    return $reporte;
}
function PorGenero($biblioteca)
{
    $generos = [];
    foreach ($biblioteca as $libro) {
        $genero = $libro['genero'];
        $generos[$genero] = array_filter($biblioteca, function ($libro) use ($genero) {
            return $libro['genero'] == $genero;
        });
    }
    return $generos;
}
function MayorAutor($biblioteca)
{
    //$autores = [];
    foreach ($biblioteca as $libro) {
        $autor = $libro['autor'];
        $autores[$autor] = array_filter($biblioteca, function ($libro) use ($autor) {
            return $libro['autor'] == $autor;
        });
    }
    $mayor = array_reduce($autores, function ($a, $b) {
        return ($a > $b) ? $a : $b;
    });

    return array_unique(array_column($mayor, 'autor'));
}
